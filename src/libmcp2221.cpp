...
// Add a device to linked list
    static void addUsbDevList(int id, struct hid_device_info *dev2) {
        ...
        if (dev != NULL) // Root is defined
        {
            ...
            // Make next item
            dev->next = (device_list_t *) malloc(sizeof(device_list_t));
            dev = dev->next;
        } else // No root, need to make it
        {
            // Make root item
            devList = (device_list_t *) malloc(sizeof(device_list_t));
            dev = devList;
        }
        ...
        // Path
        dev->devPath = (char *) malloc(strlen(dev2->path) + 1);
        ...
        if (!dev2->serial_number)
            ...
            dev->serial = (wchar_t *) malloc((len * sizeof(wchar_t)) + sizeof(wchar_t));
    ...
    static mcp2221_error doUSBget(hid_device *handle, void *data) {
        ...
        if (res != REPORT_SIZE) {
            if (res < 0)
                ...
                debug_printf(
                        "ERR (get): Data error, returned report length (%d) does not match requested length (%u)\n",
                        res, HID_REPORT_SIZE);
    ...
    static mcp2221_error USBget(mcp2221_t *device, void *data) {
        if (!device)
            return MCP2221_INVALID_ARG;
        return doUSBget((hid_device *) device->handle, data);
    }

    static mcp2221_error USBsend(mcp2221_t *device, void *data) {
        if (!device)
            return MCP2221_INVALID_ARG;
        return doUSBsend((hid_device *) device->handle, data);
    }
...
// Similar to getDescriptor but a new report is created instead of using an existing one
    ...
    static mcp2221_error getUSBInfo(mcp2221_t *device) {
        ...
        if ((res = setReport(device, report, USB_CMD_READFLASH)) != MCP2221_SUCCESS)
            return res;
        else if ((res = getDescriptor(device, report, device->usbInfo.manufacturer, FLASH_SECTION_USBMANUFACTURER)) !=
                 MCP2221_SUCCESS)
            return res;
        else if ((res = getDescriptor(device, report, device->usbInfo.product, FLASH_SECTION_USBPRODUCT)) !=
                 MCP2221_SUCCESS)
            return res;
        else if ((res = getDescriptor(device, report, device->usbInfo.serial, FLASH_SECTION_USBSERIAL)) !=
                 MCP2221_SUCCESS)
            return res;
...
// Open handle to device
    static mcp2221_t *open(const char *devPath) {
        if (!devPath)
            return NULL;
        ...
        // Store device info
        mcp2221_t *device = (mcp2221_t *) calloc(1, sizeof(mcp2221_t));
    ...
    int LIB_EXPORT mcp2221_find(int vid, int pid, wchar_t *manufacturer, wchar_t *product, wchar_t *serial) {
        ...
        while (currentDevice) {
            debug_printf("Device Found\n  type: %04hx %04hx\n  path: %s\n  serial_number: %ls",
                         currentDevice->vendor_id, currentDevice->product_id, currentDevice->path,
                         currentDevice->serial_number ? currentDevice->serial_number : L"(NONE)");
            debug_printf("\n");
...
// Open first MCP2221 found
    ...
    mcp2221_t *LIB_EXPORT mcp2221_open_device(const char *path) {
        return open(path);
    }
...
// Close handle
    ...
    mcp2221_error LIB_EXPORT mcp2221_getClockOut(mcp2221_t *device, mcp2221_clkdiv_t *div, mcp2221_clkduty_t *duty) {
        ...
        res = doTransaction(device, report);
        if (res == MCP2221_SUCCESS) {
            *div = (mcp2221_clkdiv_t) (report[5] & 0x07);
            *duty = (mcp2221_clkduty_t) (report[5] & 0x18);
        }
        return res;
    }
    ...
    mcp2221_error LIB_EXPORT mcp2221_getDAC(mcp2221_t *device, mcp2221_dac_ref_t *ref, int *value) {
        ...
        if (res == MCP2221_SUCCESS) {
            ...
            if (!(temp & 0x01) &&
                (temp & 0x06)) // VDD is selected for ref voltage, but an internal voltage reference is still set
                temp = MCP2221_DAC_REF_VDD;
            *ref = (mcp2221_dac_ref_t) temp;
            *value = report[6] & 0x1F;
        }
        return res;
    }
    ...
    mcp2221_error LIB_EXPORT mcp2221_getADC(mcp2221_t *device, mcp2221_adc_ref_t *ref) {
        ...
        res = doTransaction(device, report);
        if (res == MCP2221_SUCCESS)
            *ref = (mcp2221_adc_ref_t) ((report[7] >> 2) & 7);
        return res;
    }
    ...
    mcp2221_error LIB_EXPORT mcp2221_getInterrupt(mcp2221_t *device, mcp2221_int_trig_t *trig) {
        ...
        res = doTransaction(device, report);
        if (res == MCP2221_SUCCESS)
            *trig = (mcp2221_int_trig_t) (report[7] >> 5);
        return res;
    }
    ...
    mcp2221_error LIB_EXPORT mcp2221_getGPIO(mcp2221_t *device, mcp2221_gpioconfset_t *confGet) {
        ...
        if (res == MCP2221_SUCCESS) {
            for (int i = 0; i < MCP2221_GPIO_COUNT; i++) {
                ...
                confGet->conf[i].mode = (mcp2221_gpio_mode_t) (val & 7);
            }
        }
    ...
    mcp2221_error LIB_EXPORT mcp2221_readGPIO(mcp2221_t *device, mcp2221_gpio_value_t values[MCP2221_GPIO_COUNT]) {
        ...
        if (res == MCP2221_SUCCESS) {
            for (int i = 0; i < MCP2221_GPIO_COUNT; i++)
                values[i] = (mcp2221_gpio_value_t) (report[(i * 2) + 2]);
        }
        return res;
    }
...
*/
        NEW_REPORT(report);
        mcp2221_error res;
        if ((res = (mcp2221_error) saveReport(device, report)) != MCP2221_SUCCESS)
            return res;
    ...
    mcp2221_error LIB_EXPORT mcp2221_saveClockOut(mcp2221_t *device, mcp2221_clkdiv_t clkdiv, mcp2221_clkduty_t duty) {
        ...
        if (clkdiv != (report[5] & 0x07) || duty != (report[5] & 0x18)) {
            NEW_REPORT(reportUpdate);
            saveReportUpdate(report, reportUpdate);
            reportUpdate[3] = (uint8_t) clkdiv | (uint8_t) duty;
            res = doTransaction(device, reportUpdate);
        }
    ...
    mcp2221_error LIB_EXPORT mcp2221_loadADC(mcp2221_t *device, mcp2221_adc_ref_t *ref) {
        ...
        if ((res = (mcp2221_error) saveReport(device, report)) != MCP2221_SUCCESS)
            return res;
        *ref = (mcp2221_adc_ref_t) ((report[7] & 0x1C) >> 2);
        return res;
    }
    ...
    mcp2221_i2cWrite(mcp2221_t *device, int address, void *data, int len, mcp2221_i2crw_t type) {
        address <<= 1;
    ...
    mcp2221_error LIB_EXPORT mcp2221_i2cState(mcp2221_t *device, mcp2221_i2c_state_t *state) {
        ...
        res = doTransaction(device, report);
        if (res == MCP2221_SUCCESS)
            *state = (mcp2221_i2c_state_t) report[8];
        return res;
    }
